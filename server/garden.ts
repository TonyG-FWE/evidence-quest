...
export interface GardenRequest {contract:1;requestId:string;activity:string;revision:number;draftRevision:number;text:string;exposed:string[];disputed?:string;}
...
const components=new Map(Object.values(sources).flatMap(s=>s.paragraphs.map((text,i)=>['GA.SRC.'+s.id.toUpperCase()+'.'+(i+1),text] as const)));
...
export function gardenRequestBody(q:GardenRequest,model:string){
 const schema={type:'object',properties:{status:{type:'string',enum:['supported','clarify','contradiction','uncertain']},feedback:{type:'string'},scene:{type:['string','null'],enum:['bread','thanks','both',null]},refs:{type:'array',items:{type:'string'}}},required:['status','feedback','scene','refs'],additionalProperties:false};
