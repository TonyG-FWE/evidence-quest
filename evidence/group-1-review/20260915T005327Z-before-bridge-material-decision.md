...
Started September 14, 2026 (America/New_York). Updated September 14, 2026, 20:41 EDT (2026-09-15 00:41 UTC). Status: **IN DISCUSSION; SEED ORIGIN AND MARA'S ROLE AGREED; BRIDGE MATERIAL SOURCE PROPOSED**.
...
## Current issue for Tony: where the bridge materials come from

**Status: PROPOSAL ONLY.** This follows the recommended river-to-garden discussion order. Bridge materials can be settled independently of the unresolved bakery chronology; a shared resource rule between them is not inferred.

### Existing agreement and missing connection

The [September 14 boat/bridge correction](BOAT-BRIDGE-CORRECTION-20260914.md) establishes two separate wooden bridge sections and a separate seed boat. [D037](D037-OBJECTS-ACTIONS-AND-RESPONSES.md) establishes posts and fastening ropes. The revised note only says the boat and sections are kept beside the river. It does not identify who supplied or owns the sections, why they are available, or permission to use them. GA-B04/B05/B08 and GA-D01 record the missing material source and collection/use sequence.
...
| Proposal | Consequence for the adventure |
|---|---|
| **Recommended: recover the usable sections of the storm-damaged garden footbridge.** | Connects the visible damage to material recovery and repair. The two usable sections are reachable on Pip's starting bank. Unusable debris must be distinguishable. Reuse after a failed crossing remains possible, but the actual collapse/recovery sequence still needs its own specification. |
| Use replacement sections stored beside the crossing for maintenance. | Gives a straightforward supply source with a clear storage place. The broken bridge remains the obstacle, but its debris supplies no usable material; the story must explain the separate replacement stock. |

Proposed ownership and permission for the recommended version: Grandma maintains this garden footbridge. Its usable recovered sections can be reused, and she keeps fastening ropes in a clearly marked maintenance box on the starting bank. A standing notice gives visitors permission to use these bridge materials. The notice must not require reaching Grandma first or speaking to Mara; alternate visit orders remain viable. Grandma's maintenance ownership, the box and its notice are new proposals, not deductions from the old note.

### Concrete proposed situation
...
The storm did not create this notice or give Grandma unexplained knowledge of today's damage. It is proposed standing permission/instruction, present before the damage.
...
| Field | Proposed detail or existing constraint |
|---|---|
| Location/time | Starting bank, before first crossing; all required initial bridge materials are reachable there |
| Owner/source/permission | Proposed: Grandma's garden footbridge; storm-displaced reusable sections; maintenance ropes with standing permission beside the crossing |
...
| Unresolved dependencies | Material-source choice awaits Tony; exact maintenance ownership/notice form follows that choice. Posts, handling, collapse recovery, seed-boat purpose and any bakery material relationship are still open |

Question for this step: use recovered sections of Grandma's damaged garden footbridge, with her maintenance ropes available beside the crossing, or use stored replacement sections? Only this connected material-source proposal is being considered now.
