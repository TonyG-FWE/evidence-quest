import type { CaseState, Draft, Exposure, Order, Point, Preferences, Run } from '../../contracts/types.js';
...
export type Command =
 ...
 |{type:'SELECT_TILE';tile:Order[number]}|{type:'EDIT_RAIL';operation:'insert'|'replace'|'swap'|'return'|'before'|'after'|'left'|'right';index:number}|{type:'CANCEL_TILE'}
 ...
 |{type:'ART_FAILED'}|{type:'CANVAS_FAILED'}|{type:'TOAST_START'}|{type:'TOAST_DONE';actionId:string}|{type:'DISMISS_GUIDE';guide:'openingDismissed'|'movementDismissed'|'ariInvitationDismissed'}|{type:'BACKGROUND'};
export interface Envelope {id:string;ids:string[];visitId:string;caseId:string;command:Command;}
export type Effect={kind:'timer';ms:number;command:Command}|{kind:'save';draft?:boolean}|{kind:'preferences'};
