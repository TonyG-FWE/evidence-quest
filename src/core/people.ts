...
export function deliver(m:Mutation,delivery:Delivery){
 ...
 else if(valid.some(r=>r.startsWith('E4.')))ct=ari?ariAccount(m):actor==='ACT.REMY'?'CT.REMY.REQUEST':['docked','projecting'].includes(c.physical.loop.mode)?'CT.NPC.DETAILS':'CT.JO.BORROW';
...
export function talk(m:Mutation,topic:string){
 ...
 if(topic==='CT.TALK.ROLE')ct='CT.GOAL.ASSIGNMENT';
 else if(topic==='CT.TALK.LOOP')ct=actor==='ACT.JO'?['docked','projecting'].includes(p.loop.mode)?'CT.JO.DOCKED':'CT.JO.BORROW':ariAccount(m);
  ...
  observe(m,'help-displayed',{contentIds:[ct],origin:'npc',assistanceLevel:3});
...
export function help(m:Mutation,direct=false,noteHelp=false){
 const topic=m.s.runtime.view.topic==='story'?'story-plan':m.s.case.selectedLead??'where-loop',context=contextOf(m.s,topic);
 const prior=m.s.case.coachingHistory.filter(h=>h.displayedSeq!==null&&h.context.puppet.seed===context.puppet.seed&&h.context.puppet.pip===context.puppet.pip&&h.context.runId===context.runId&&h.contentIds.some(ct=>ct.startsWith('CT.HINT.STORY_')||ct.startsWith('CT.HINT.PROMISE_'))).length;
 const answer=prepared(context,{direct,noteHelp,repeat:prior,topicSelected:m.s.runtime.view.topic!==undefined||m.s.case.selectedLead!==null});
 const requestId=allocate(m),draft=m.s.case.drafts.find(d=>d.id===(topic==='story-plan'?'coach-story':'coach-search'))!;
 if(Array.from(draft.text).length>600){caption(m,['CT.UI.LIMIT']);return;}
 const submitted=draft.text.trim()?record(m,'coaching-submission',topic==='story-plan'?'story':'search',draft.text,draft.selectedRefs,null):null;
 observe(m,'help-requested',{requestId,recordId:submitted?.id??null,origin:direct?'authored-direct':'authored-topic'});
 const seq=observe(m,'help-displayed',{requestId,recordId:submitted?.id??null,contentIds:answer.contentIds,refs:answer.refs,assistanceLevel:answer.level,origin:direct?'authored-direct':'authored-topic'});
 m.s.case.coachingHistory.push({requestId,submittedRecordId:submitted?.id??null,context,contentIds:answer.contentIds,selection:null,origin:direct?'authored-direct':'authored-topic',displayedSeq:seq});
 setView(m,{page:'help',topic:topic==='story-plan'?'story':'search',cts:answer.contentIds});m.s.session.coach.status='displayed';m.s.session.coach.winner=direct?'direct':'fallback';
}
