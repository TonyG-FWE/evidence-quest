...
export function record(m:Mutation,kind:IdeaRecord['kind'],topic:'search'|'story',text:string,refs:string[],recipient:IdeaRecord['recipient'],options:{allowEmpty?:boolean;previousRecordId?:string|null}={}):IdeaRecord|null{
 if(kind!=='evidence-delivery'&&((!options.allowEmpty&&!text.trim())||Array.from(text).length>600)){caption(m,[text.trim()?'CT.UI.LIMIT_RECORD':'CT.UI.ADD_IDEA']);return null;}
 const id=allocate(m),previous=options.previousRecordId!==undefined?m.s.case.records.find(r=>r.id===options.previousRecordId):[...m.s.case.records].reverse().find(r=>r.kind===kind&&r.topic===topic&&r.recipient===recipient);
