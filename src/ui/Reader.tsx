...
export function Reader({state,store}:{state:State;store:Store}){
 ...
 const granted=source.parts.filter(p=>available(state.case,p.refId)&&!(id==='E3'&&state.case.physical.objects.noticeFlat&&p.refId==='CT.OBJ.NOTICE_PARTIAL'));
 const component=id==='E2'&&available(state.case,'E2.a/frame1')?(view.ref&&view.ref!=='frames'?view.ref:`E2.a/frame${view.frame??1}`):granted.find(p=>p.refId===view.ref)?.refId??granted[0]?.refId;
 useEffect(()=>{
  ...
  else if(view.ref===component){const part=parts.get(component),passage=panel.querySelector<HTMLElement>(`[data-content-id="${part?.ctId}"]`),word=passage?.querySelector<HTMLElement>(`[data-start="${part?.spans[0]?.[0]}"]`);(word??passage)?.scrollIntoView({block:'nearest'});}
 ...
 return <article className="reader">
  ...
  {granted[0]&&<p className="source-provenance">{provenance(component??granted[0].refId)}</p>}
  ...
  {isFile?<>
   ...
   {view.ref==='E2.b'?<><NoticePhoto/>{display('E2.b')}</>:view.ref==='E2.a/description'?<><SourceMetadata refId="E2.a/description"/>{display('E2.a/description')}</>:view.ref==='E2.c'?<><SourceMetadata refId="E2.c"/>{display('E2.c')}</>:<>
    ...
    <h3>{copy('CT.MEDIA.MESSAGE')}</h3><SourceMetadata refId="E2.c"/>{display('E2.c')}
   </>}
  </>:<>
   ...
   {[...new Set(granted.map(p=>p.ctId))].map(ct=>{
    ...
    const context=group.map(p=>historical(p.refId)).find(Boolean);
    return <section key={ct}>{context&&<p className="source-provenance">{context}</p>}{id==='E5'&&<p className="source-provenance">{provenance(group[0]!.refId)}</p>}<Passage ct={ct} refs={group.map(p=>p.refId)} access={access} store={store} onWord={define} {...(range?{range}:{})}/></section>;
   })}
  </>}
  <div className="actions"><Button ct="CT.NOTES.COMPARE" onClick={()=>store.send({type:'VIEW',view:{page:'compare'}})}/><Button ct="CT.IDEA.FIELD" onClick={()=>store.send({type:'VIEW',view:{page:'idea'}})}/></div><p className="muted">{copy('CT.ACCESS.SOURCE')}</p>
 </article>;
}
