...
export function StoryCanvas({s,model=false}:{s:State;model?:boolean}){
 ...
 useEffect(()=>{
  ...
  let assetFrame:number|null=null;paint.current=draw;const assets=new Assets(()=>{if(assetFrame===null)assetFrame=requestAnimationFrame(()=>{assetFrame=null;draw();});},()=>{}),resize=new ResizeObserver(draw);resize.observe(el);draw();return()=>{resize.disconnect();if(assetFrame!==null)cancelAnimationFrame(assetFrame);assets.dispose();};
 },[model]);
