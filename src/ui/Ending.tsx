...
import {ReadingRecap,showReading} from './Experience.js';
export function Ending({s,open}:{s:State;open:(view:View)=>void}){
 ...
 return <><h2 tabIndex={-1} data-focus-heading>{copy(recap?'CT.ENDING.RECAP':reactions?'CT.ENDING.AFTER':'CT.ENDING.TITLE')}</h2>
  {reactions?<>
   <div className="reaction-art"><img className="reaction-room" src={assetUrl(`ASSET.ENV.${step===0?'CY':'MD'}.BACKPLATE`)} alt=""/><Sprite className="reaction-person" id={`ASSET.ACT.${step===0?'REMY':'ARI'}`} variant="celebrate" alt={step===0?'Remy':'Ari'}/></div>
   ...
   <p>{copy('CT.GOAL.AFTER')}</p><p>{copy(recap?'CT.ENDING.FACT':'CT.ENDING.LIGHT')}</p>
   {recap&&<ReadingRecap s={s}/>}<Button onClick={()=>showReading(s,'READ.ENDING')}>Read our ending with the crew</Button>
