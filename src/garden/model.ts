...
import {freshBakery,applyBakery,settleBakery,bakeryReady,bakeryAction,bakeryDurations,bakeryActionText,bakeryInstruction,nearBakery,near as bakeryNear,BAKERY_SOL,TILE_SHELF,WORKSHOP_DOOR,type BakeryState,type BakeryAction,type BakeryStep} from './bakery.js';
import {SerializedQueue} from '../core/serialized.js';
import {sources,words,sourcePrefix} from './content.js';
...
import {preparedEndings,endingLines} from './chapterContent.js';
import {freshRiver,collapseResult,snapSections,sectionAtEnd,anchoredSection,atMooring,boatWater,ropeCount,LAUNCH,LANDING,type RiverState} from './river.js';
const sourceIds=new Set(Object.values(sources).flatMap(source=>source.paragraphs.flatMap((p,i)=>{const id=sourcePrefix(source.id)+(i+1);return [id,...words(p).map((_,n)=>id+'.W'+(n+1))];})));
...
export type Panel=SourceId|'festival'|'endingWords'|'birdTalk'|'grandma'|'backpack'|'help'|'pause'|'complete'|'writing'|'planner'|'gathering'|'lanterns'|'studio'|null;
export interface Chapter {
 ...
 seed:'pip'|'boat'|'grandma'|'soil';bloomed:boolean;maraHeard:boolean;page:'mara'|'pip'|'grandma';grandmaHeard:boolean;
...
export interface GardenState {
 ...
 chapter:Chapter;ready:boolean;mode:'walk'|'arrange'|'boat'|'mara-story'|'bakery-repair';boatTarget:Point|null;panel:Panel;selection:'a'|'b';preview:{a:Section;b:Section}|null;
...
export type Command=PlaybackCommand
 ...
 |{type:'COLLECT_ROPES'}|{type:'LAUNCH_BOAT'}|{type:'STEER';point:Point}|{type:'DOCK_SEED'}|{type:'UNLOAD_SEED'}
...
export const MARA={x:-3.5,z:-4.0},GRANDMA={x:4.4,z:3.6},PLANT={x:3.75,z:3.55},CROSSING={x:-1.98,z:.8};
...
export function blocksFerryLane(p:Section){const x=Math.abs(Math.cos(p.rotation))*.775+Math.abs(Math.sin(p.rotation))*.56,z=Math.abs(Math.cos(p.rotation))*.56+Math.abs(Math.sin(p.rotation))*.775;return Math.abs(p.x)<1.74+x&&Math.abs(p.z-FERRY_WEST.z)<.7+z;}
export const GRANDMA_APPROACH={x:3.25,z:3.20};
export const TREE_POSITIONS=[[-5.7,1.7,1.4],[-5.6,4.7,1.25],[-4.2,5.3,1],[-6,-1.5,1.05],[5.6,-3.8,1.25],[5.9,-.5,1.3],[6.1,4.9,1.2],[2.15,5.3,.9]] as const;
...
export function freshChapter(runId:string):Chapter{return {version:3,content:'garden-chapter-3',story:freshStory(),river:freshRiver(),mara:freshMara(),bakery:freshBakery(),gathering:freshGathering(),conversations:freshConversations(),runId,revision:0,started:false,pip:{x:-3.5,z:-1.8},sections:structuredClone(STAGED_SECTIONS),ferrySide:'west',joined:false,west:false,east:false,crossed:false,seed:'pip',bloomed:false,maraHeard:false,page:'mara',grandmaHeard:false,exposed:[],assistance:[],history:[],reading:{},reducedMotion:false,largeText:false};}
export function initialGarden(runId:string):GardenState{return {playback:null,bakeryPreview:null,maraTarget:null,maraParent:null,chapter:freshChapter(runId),ready:false,mode:'walk',boatTarget:null,panel:null,panelTrail:[],restoreFocus:null,activity:null,actionParent:null,viewDrafts:{memoryMoment:'planting',endingScene:'bread',plan:null,disclosures:{}},selection:'a',preview:null,route:[],keys:[],action:null,pendingBloom:false,notice:'',requested:null,save:'loading',savedRevision:0,background:false,viewLost:false};}
export function riverHalf(z:number){return 2.2-.8*Math.max(0,Math.min(1,(z+2)/5));}
...
export function reaches(c:Chapter,end:'west'|'east'){
 if(!c.joined)return false;const p=bridgeCenter(c),edge=p.x+(end==='west'?-1.55:1.55),bank=(end==='west'?-1:1)*riverHalf(p.z);
...
export const bridgeReady=(c:Chapter)=>c.joined&&c.west&&c.east&&reaches(c,'west')&&reaches(c,'east');
...
export function walkable(c:Chapter,p:Point){
 if(p.x< -6.25||p.x>9.75||p.z< -5.4||p.z>5.25||distance(p,PLANT)<.59||TREE_POSITIONS.some(([x,z,scale])=>distance(p,{x,z})<.18+.12*scale))return false;
 if(p.x>-5.86&&p.x<-4.34&&p.z>-5.16&&p.z<-3.84||p.x>4.36&&p.x<5.94&&p.z>1.86&&p.z<2.67)return false;
 if(p.x>6.6&&(p.z< -4.8||p.z>.75))return false;
 if(p.x>7.1&&p.x<9.45&&p.z>-3.65&&p.z<-1.15)return false;
 if(p.x>3.5&&p.x<5.25&&p.z>-4.45&&p.z<-2.8)return false;
 if(p.x>=-3.3&&p.x<=-2.04&&p.z>=-4.6&&p.z<=-3.64)return true;
 const center=bridgeCenter(c);
 if(bridgeAttemptable(c)&&Math.abs(p.z-center.z)<.38&&Math.abs(p.x)<2.1)return true;
 return Math.abs(p.x)>=riverHalf(p.z)+.18;
}
function lineClear(c:Chapter,a:Point,b:Point){const n=Math.max(1,Math.ceil(distance(a,b)/.07));for(let i=1;i<=n;i++)if(!walkable(c,{x:a.x+(b.x-a.x)*i/n,z:a.z+(b.z-a.z)*i/n}))return false;return true;}
/** Routes use the same bank/deck predicates as direct movement. */
export function routeTo(c:Chapter,to:Point):Point[]{
 if(!walkable(c,to))return [];
 const center=bridgeCenter(c),nodes=[c.pip,to,{x:-2.6,z:3},{x:2.6,z:3},{x:-3,z:-2},{x:3,z:-2},{x:-3,z:4.6},{x:3,z:4.6},{x:5,z:2.8},{x:5.45,z:-1.4},{x:6.6,z:-1.4},{x:6.7,z:-.55},{x:8.5,z:-.55},{x:9.5,z:-.55}];
 if(bridgeAttemptable(c))nodes.push({x:-1.9,z:center.z},{x:1.9,z:center.z});
 const costs=nodes.map(()=>Infinity),prev=nodes.map(()=>-1),used=new Set<number>();costs[0]=0;
 for(let k=0;k<nodes.length;k++){
  let best=-1;for(let i=0;i<nodes.length;i++)if(!used.has(i)&&(best<0||costs[i]!<costs[best]!))best=i;
  if(best<0||costs[best]===Infinity||best===1)break;used.add(best);
  for(let i=0;i<nodes.length;i++)if(i!==best&&lineClear(c,nodes[best]!,nodes[i]!)){const cost=costs[best]!+distance(nodes[best]!,nodes[i]!);if(cost<costs[i]!){costs[i]=cost;prev[i]=best;}}
 }
 if(!Number.isFinite(costs[1]))return [];const result:Point[]=[];let idx=1;while(idx>0){result.unshift(nodes[idx]!);idx=prev[idx]!;}return result;
}
function settled(s:GardenState,continueGrowth:boolean,id:string){
 ...
 if(!c.history.includes(a.id)){
  ...
  if(a.kind==='loadSeed'){c.seed='boat';c.river.boat.phase='loaded';s.mode='boat';s.notice='The seed is aboard. Launch, then steer around the rock to Grandma’s blue landing.';}
  if(a.kind==='unloadSeed'){c.seed='pip';c.river.boat={phase:'moored',position:{...LAUNCH}};s.mode='walk';s.notice='Pip takes the same seed out of the boat. You can carry it across the bridge instead.';}
  if(a.kind==='receiveSeed'){c.seed='grandma';c.river.boat={phase:'moored',position:{...LANDING}};c.ferrySide='east';c.river.soilPrepared=true;c.history.push('F');s.mode='walk';s.notice='Grandma takes the seed and prepares the soil. She will wait for Pip to plant it with her.';}
  ...
  if(a.kind==='collapse'){const result=collapseResult(c);c.sections=result.sections;c.joined=false;c.pip=result.pip;c.story.bridgeFailures++;s.notice='The loose bridge comes apart. Pip steps safely back. '+(c.west?'The dock-side rope holds its section.':c.east?'The garden-side rope holds its section.':'Both sections settle in the shallow water beside their banks.')+' Recover the same pieces, rejoin them and secure both ends.';}
 ...
 if(!s.panel&&!s.actionParent&&(a.kind==='tilePlacement'&&c.bakery.stage==='sealed'||['flourCheck','mixDough','shapeLoaves','bakeBread','bakeUnshaped','thankSol'].includes(a.kind)))enterReader(s,'bakery');
 ...
 if(continueGrowth&&s.pendingBloom&&!['maraTell','birdIntro','birdConsent','tapePickup','birdDeparture','maraReturn','dockService'].includes(a.kind)){s.action={id:id+'-growth',kind:'bloom',elapsed:0,duration:1800,from:{...PLANT},hasSeed:false};s.pendingBloom=false;}
}
function stop(s:GardenState,id:string){s.route=[];s.keys=[];s.boatTarget=null;s.maraTarget=null;s.bakeryPreview=null;if(s.chapter.mara.scene)s.chapter.mara.scene.preview=null;s.preview=null;settled(s,false,id);}
function start(s:GardenState,kind:Action['kind'],id:string,placement?:'gap'|'beside',moment?:'planting'|'gathering'){
 if(s.action)return;stop(s,id);s.action={id,kind,...(placement?{placement}:{}),...(moment?{moment}:{}),elapsed:0,duration:gatheringAction(kind)?gatheringDurations[kind]:bakeryAction(kind)?bakeryDurations[kind]:kind==='keepMemory'?1800:kind==='dockService'?7000:kind==='maraTell'||kind==='maraReturn'?2000:kind==='birdDeparture'?2500:kind==='birdConsent'?1500:kind==='arrival'?10000:kind==='collapse'?3000:kind==='receiveSeed'?5000:kind==='ferry'?1200:kind==='plant'?3000:kind==='bloom'?1800:1000,from:{...(kind==='collapse'||kind==='keepMemory'||bakeryAction(kind)?s.chapter.pip:ferryPosition(s.chapter))},hasSeed:kind==='loadSeed'||kind==='receiveSeed'||kind==='unloadSeed'};
...
function previewAt(s:GardenState,p:Point,magnet=true){
 ...
 if((c.joined&&(c.west||c.east))||(['west','east'] as const).some(end=>c[end]&&sectionAtEnd(c,end)===s.selection)){s.notice='That section is held by a rope. Release its attachment before moving it.';return;}
...
function commitPlacement(s:GardenState){
 ...
 s.chapter.sections=sections;s.preview=null;s.notice=s.chapter.joined?'The joined sections are placed. Fasten each end where it reaches a bank.':'Section placed. Bring the short ends together, then join the sections.';
}
export function gardenReduce(state:GardenState,command:Command,id:string):GardenState{
 ...
 if(state.chapter.gathering.turn&&!['PLAYBACK','STORY','BOOT','OPEN','INSPECT_TEXT','CLOSE','TICK','INTERRUPT','FOREGROUND','SETTING','SAVE_STATUS','VIEW_LOST','NEW','READ_POSITION','HELP_USED','EXPOSE_WORDS','EXPOSE','NOTICE','DISCLOSURE'].includes(command.type))return state;
 if(state.mode==='bakery-repair'&&!['BOOT','CANCEL','BAKERY_STEP','TILE_PREVIEW','TICK','OPEN','INSPECT_TEXT','CLOSE','INTERRUPT','FOREGROUND','SETTING','SAVE_STATUS','VIEW_LOST','NEW','READ_POSITION','HELP_USED','EXPOSE_WORDS','EXPOSE','NOTICE','DISCLOSURE'].includes(command.type))return state;
 if(state.mode==='mara-story'&&!['BOOT','CANCEL','MARA_GO','MARA_STEP','TAPE_PREVIEW','KEY','TICK','OPEN','INSPECT_TEXT','CLOSE','INTERRUPT','FOREGROUND','SETTING','SAVE_STATUS','VIEW_LOST','NEW','READ_POSITION','HELP_USED','EXPOSE_WORDS','EXPOSE','NOTICE','DISCLOSURE'].includes(command.type))return state;
 ...
 if(state.chapter.seed==='boat'&&['GO','TALK','ARRANGE','TRY_CROSS','STORY','BAKERY_STEP','TILE_PREVIEW','FERRY','PLANT','BLOOM','TAKE_PAGE','REPORT','COLLECT_ROPES','BACK'].includes(command.type))return {...state,notice:'The seed is still aboard. Dock at Grandma’s landing, or return to the launch and unload before guiding Pip.'};
 ...
 const s=structuredClone(state),c=s.chapter;let changed=true;
 switch(command.type){
 case 'PLAYBACK':controlPlayback(s,command,id);break;
 case 'BOOT':if(command.chapter)s.chapter=structuredClone(command.chapter);if(s.chapter.story.phase==='arriving'&&!connectedGathering(s.chapter)){s.chapter.story.phase='welcome';s.chapter.revision++;}if(s.chapter.joined&&!bridgeReady(s.chapter)&&Math.abs(s.chapter.pip.x)<riverHalf(s.chapter.pip.z)+.1){const result=collapseResult(s.chapter);s.chapter.sections=result.sections;s.chapter.joined=false;s.chapter.pip=result.pip;s.chapter.story.bridgeFailures++;s.chapter.revision++;}if(s.chapter.mara.scene?.preview){s.chapter.mara.scene.preview=null;s.chapter.revision++;}s.ready=true;s.save=command.failure??(command.pendingWrite?'saving':'saved');if(command.chapter&&s.chapter.revision!==command.chapter.revision)s.save='saving';s.savedRevision=s.save==='saving'?-1:s.chapter.revision;s.action=null;s.route=[];s.keys=[];s.boatTarget=null;s.preview=null;s.bakeryPreview=null;s.panel=null;s.mode=s.chapter.mara.scene?'mara-story':s.chapter.seed==='boat'?'boat':['gap','misplaced','delivered'].includes(s.chapter.bakery.stage)&&bakeryNear(s.chapter.pip,BAKERY_SOL)?'bakery-repair':'walk';s.maraTarget=null;s.maraParent=null;if(s.chapter.mara.scene)s.chapter.mara.scene.preview=null;s.pendingBloom=s.chapter.seed==='soil'&&!s.chapter.bloomed;if(s.chapter.mara.scene&&!s.chapter.mara.scene.introduced){s.action={id:id+'-bird-intro',kind:'birdIntro',elapsed:0,duration:s.chapter.reducedMotion?1:2200,from:{...BIRD_BOY},hasSeed:false};}break;
 ...
 case 'OPEN':if(command.panel==='planner'&&!c.story.maraReported)break;if(command.panel==='writing'&&(c.story.phase!=='planning'||!c.story.laterKnown))break;if(command.panel==='bakery'&&!c.bakery.met)break;if(command.panel==='sol'&&!bakeryReady(c))break;if(s.mode==='bakery-repair'&&!['festival','help','backpack','pause',...Object.keys(sources)].includes(command.panel??''))break;if(command.panel==='birdTalk'&&(s.mode!=='mara-story'||c.mara.scene?.stage!=='ask'||!nearBird(c.mara.scene.position)))break;if(s.mode==='mara-story'&&!['festival','birdTalk','help','backpack','pause',...Object.keys(sources)].includes(command.panel??''))break;if(command.panel==='complete'){if(c.story.ending&&!s.activity){stop(s,id);enterReader(s,'studio',command.focus);s.notice='';}break;}if(command.panel==='endingWords'&&!c.story.ending)break;if(s.activity&&!['festival','help','pause','backpack',...(s.panel?Object.keys(sources):[]),...(s.activity.kind==='ending-presentation'?['endingWords']:s.activity.kind==='lantern-inspect'?[lanternRecord(c,s.activity.lantern).page].filter(Boolean):[])].includes(command.panel??''))break;if(['finale','breadEnding','thanksEnding','notice'].includes(command.panel??'')||command.panel==='story'&&c.page==='mara'&&!c.story.records.mara&&c.story.phase!=='mara'||command.panel==='mara'&&!c.maraHeard||command.panel==='report'&&!conversationReady(c,'mara')||command.panel==='sol'&&!c.story.metSol||command.panel==='later'&&!c.story.laterKnown||command.panel==='empty'&&!(c.story.records.grandma||c.gathering.pageComplete)||command.panel==='studio'&&!c.story.ending||['picnic','duet','lanterns'].includes(command.panel??'')&&!c.crossed)break;stop(s,id);s.notice='';enterReader(s,command.panel,command.focus);break;
 ...
 case 'TICK':{
  ...
  if(s.action){s.action.elapsed+=dt;if(s.action.elapsed>=s.action.duration)settled(s,true,id);break;}
   ...
   if(s.activity){if(!s.panel)s.activity.elapsed=Math.min(6000,s.activity.elapsed+dt);break;}
  if(s.mode==='boat'){
   ...
   const goal=h||v?{x:p.x+h*.93+v*.37,z:p.z-h*.37+v*.93}:s.boatTarget;
   if(!goal)break;const d=distance(p,goal),step=Math.min(d,dt*.0014),next={x:p.x+(goal.x-p.x)/(d||1)*step,z:p.z+(goal.z-p.z)/(d||1)*step};
   if(boatWater(next)){c.river.boat.position=next;if(d<=step)s.boatTarget=null;s.notice=atMooring(c,'east')?'You reached Grandma’s blue landing. Give her the seed.':atMooring(c,'west')?'The boat is at Pip’s launch. You can unload the seed here.':'Steer the seed boat around the rock to Grandma’s blue landing.';}
   else{s.boatTarget=null;s.keys=[];s.notice=atMooring(c,'east')?'You reached Grandma’s blue landing. Give her the seed.':atMooring(c,'west')?'The boat is at Pip’s launch. You can unload the seed here.':Math.hypot(next.x,next.z-.8)<.45?'The rock blocks that way. The seed is safe aboard. Steer around it.':'This is not a landing. The seed is still aboard. Follow the water to the marked blue landing, or return to Pip’s launch.';}break;
  }
  ...
  if(h||v){const mag=Math.hypot(h,v),step=dt*.003;next={x:next.x+(h*.93+v*.37)/mag*step,z:next.z+(-h*.37+v*.93)/mag*step};if(walkable(c,next))c.pip=next;else s.notice=c.joined&&!bridgeReady(c)?LOOSE:'The water is too deep to walk through. Stay on the bank or the finished crossing.';}
  else if(s.route.length){const p=s.route[0]!,d=distance(c.pip,p),step=dt*.003;if(d<=step){c.pip={...p};s.route.shift();if(!s.route.length)s.notice=s.requested?'Pip is here. Choose the nearby action.':'';}else{next={x:c.pip.x+(p.x-c.pip.x)/d*step,z:c.pip.z+(p.z-c.pip.z)/d*step};if(walkable(c,next))c.pip=next;else{s.route=[];s.notice='That path is blocked.';}}}
  if(c.bakery.stage==='escorting'&&c.pip.x>=3&&c.pip.z>=-2.4&&c.pip.z<=.7&&distance(before,c.pip)>.0001&&distance(c.bakery.rina,c.pip)>.65){const path=routeTo({...c,pip:c.bakery.rina},c.pip),target=path[0];if(target){const d=distance(c.bakery.rina,target),step=Math.min(d,dt*.0032);c.bakery.rina={x:c.bakery.rina.x+(target.x-c.bakery.rina.x)/(d||1)*step,z:c.bakery.rina.z+(target.z-c.bakery.rina.z)/(d||1)*step};}}
  if(!c.crossed&&bridgeAttemptable(c)&&!bridgeReady(c)&&Math.abs(c.pip.x)<riverHalf(c.pip.z)+.1){start(s,'collapse',id);break;}
 ...
 case 'ARRANGE':if(s.action||distance(c.pip,CROSSING)>1.6||c.crossed){s.notice=c.crossed?'The crossing stays here for everyone to use.':'Bring Pip beside the bridge pieces first.';break;}stop(s,id);resetViews(s);s.panel=null;s.mode='arrange';s.notice='Move the wooden bridge sections. Pip will wait on the bank.';break;
 case 'COLLECT_ROPES':if(!s.action&&distance(c.pip,CROSSING)<=1.6){if(ropeCount(c,'box'))start(s,'ropes',id);else s.notice='The repair box is empty. Both ropes are already in use or in Pip’s backpack.';}break;
 case 'POST_PREVIEW':if(s.mode==='arrange'){const x=command.site==='narrow'?(command.side==='west'?-.775:.775):(command.side==='west'?-1.575:-.025);previewAt(s,{x,z:command.site==='narrow'?3:-2});s.notice=s.preview?'Preview at the '+command.site+' crossing. Choose Place section to keep it.':s.notice;}break;
 ...
 case 'CANCEL':s.bakeryPreview=null;s.maraTarget=null;if(c.mara.scene)c.mara.scene.preview=null;s.preview=null;s.route=[];s.keys=[];s.boatTarget=null;break;
 ...
 case 'JOIN':{if(s.mode!=='arrange'||c.joined)break;const a=c.sections.a,b=c.sections.b;if(Math.abs(a.z-b.z)>.17||Math.abs(Math.abs(a.x-b.x)-1.55)>.17||Math.abs(Math.sin(a.rotation))>.01||Math.abs(Math.sin(b.rotation))>.01||Math.abs(a.rotation-b.rotation)>.01){s.notice='Bring the short ends of the bridge sections together. Turn the loose section to face the same way as the other section.';break;}const heldB=c.river.attachments.west==='b'||c.river.attachments.east==='b',key=heldB?'a':'b',origin=heldB?b:a,other=heldB?a:b,joinedPart={x:origin.x+(other.x>origin.x?1.55:-1.55),z:origin.z,rotation:origin.rotation};if(Math.abs(joinedPart.x)>5||blocksFerryLane(joinedPart)){s.notice='Move the sections farther from the seed boat’s route before joining them.';break;}if((c.river.attachments.west===key||c.river.attachments.east===key)&&distance(joinedPart,other)>.001){s.notice='Release an attachment before adjusting this section.';break;}c.sections[key]=joinedPart;c.joined=true;s.notice='The sections are joined. Does the crossing reach both banks?';break;}
 case 'FASTEN':if(s.mode!=='arrange'||c.crossed)break;if(c.river.ropeLocations[command.end]!=='pip'){s.notice='Take the repair rope from Grandma’s maintenance box first.';break;}if(!reaches(c,command.end)||(command.atZ!==undefined&&Math.abs(bridgeCenter(c).z-command.atZ)>.22)){s.notice=GAP;break;}c.river.attachments[command.end]=sectionAtEnd(c,command.end);c[command.end]=true;c.river.ropeLocations[command.end]='attached';s.notice=bridgeReady(c)?'Both ends are secure. Return to Pip, then walk across.':LOOSE;break;
 case 'ADJUST':if(s.mode==='arrange'&&!c.crossed){for(const end of ['west','east'] as const)if(c[end])c.river.ropeLocations[end]='pip';c.west=false;c.east=false;c.river.attachments={west:null,east:null};s.notice='The ropes are released into Pip’s backpack. Move the sections, then fasten them again.';}break;
 case 'FERRY':if(s.action||s.panel||s.background||s.viewLost)break;if(c.seed==='pip'&&!c.crossed){if(c.ferrySide!=='west'||distance(c.pip,CROSSING)>1.6){s.notice='Bring Pip beside the seed boat to load the seed.';break;}start(s,'loadSeed',id);break;}if(distance(c.pip,CROSSING)>1.6&&distance(c.pip,GRANDMA)>1.6){s.notice='Return to the riverbank or Grandma to watch the seed boat.';break;}start(s,'ferry',id);break;
 case 'LAUNCH_BOAT':if(s.mode==='boat'&&!s.panel&&!s.action&&c.seed==='boat'&&c.river.boat.phase==='loaded'){c.river.boat.phase='steering';s.notice='You steer the seed boat. Click water or use the arrow keys. Go around the rock to Grandma’s blue landing.';}break;
 ...
 case 'DOCK_SEED':if(s.mode==='boat'&&!s.panel&&!s.action&&!s.background&&!s.viewLost&&c.seed==='boat'){if(atMooring(c,'east'))start(s,'receiveSeed',id);else s.notice='Grandma can reach the boat at her blue landing. Steer there before giving her the seed.';}break;
 ...
 case 'PLANT':if(s.action||!c.crossed||distance(c.pip,GRANDMA)>1.5||!['pip','grandma'].includes(c.seed))break;s.panel=null;start(s,'plant',id);break;
 ...
 case 'SAVE_STATUS':s.save=command.status;if(command.revision!==undefined)s.savedRevision=command.revision;break;
...
export class GardenStore {
 ...
 constructor(initial:GardenState,private readonly allocate:()=>string=()=>crypto.randomUUID()){
  this.state=freeze(initial);this.queue=new SerializedQueue(command=>{const next=gardenReduce(this.state,command,this.allocate());if(next===this.state)return;this.state=freeze(next);for(const listener of this.listeners)listener();});
 }
