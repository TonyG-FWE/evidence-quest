...
import {BAKERY_APPROACH,BAKERY_SOL,TILE_SHELF,WORKSHOP_DOOR,bakeryInstruction,near,type BakeryStep} from './bakery.js';
export function BakeryControls({s,store}:{s:GardenState;store:GardenStore}){
 ...
 return <aside className="garden-bakery-controls" aria-label="Help at Rina’s bakery" tabIndex={-1}>
  ...
  {b.stage==='needed'&&(near(c.pip,TILE_SHELF)?<button className="g-primary" disabled={busy} onClick={()=>step('PICKUP')}>Take the spare tile</button>:<button className="g-primary" disabled={busy} onClick={()=>go(TILE_SHELF,'Rina’s tile shelf')}>Go to the tile shelf</button>)}
  ...
  {b.stage==='mixed'&&near(c.pip,b.rina,1.65)&&<button className="g-alternative" disabled={busy} onClick={()=>step('BAKE_UNSHAPED')}>Try baking the whole lump</button>}
  ...
  {b.stage==='escorting'&&<>{near(c.pip,WORKSHOP_DOOR,1.6)&&near(b.rina,WORKSHOP_DOOR,1.8)?<button className="g-primary" disabled={busy} onClick={()=>step('THANK')}>Let Rina give Sol the loaf</button>:<button className="g-primary" disabled={busy} onClick={()=>go({x:4.65,z:-1.25},'Sol’s workshop with Rina')}>Walk with Rina to the workshop</button>}{!near(c.pip,b.rina,1.65)&&<button className="g-secondary" disabled={busy} onClick={()=>go(b.rina,'Rina, who is waiting with the loaf')}>Go back to Rina</button>}<p className="g-small">Rina waits if you leave this path. The loaf stays with her.</p></>}
  {b.met&&b.stage!=='arrival'&&s.mode==='walk'&&<button className="g-secondary" disabled={busy} onClick={talk}>Talk with Rina about what happens next</button>}
