...
import {speechAttributes,speechFromElement,speechDocument} from './readingSpeech.js';
...
import {BakeryControls} from './BakeryControls.js';
...
import {ReadingContext,ReadingParagraph,ConversationReadingTools,type ReadingOrigin} from './Reading.js';
import './garden.css';
...
function PersonalReviewFullscreen(){
 ...
 return <><button className="garden-studio-review g-secondary" onClick={()=>void toggle()}>{active?'Exit full screen':'Full screen'}</button>{notice&&<span role="status">{notice}</span>}</>;
}
export function App(){
 ...
 function inspectScreen(opener:HTMLElement){
  ...
  const blocks=Array.from(root?.querySelectorAll<HTMLElement>('h1,h2,h3,p,small,blockquote,.g-speaker,[data-readable-text],.g-item strong,.garden-save-error>span,button:not(.g-word),summary,legend')??[]).filter(n=>n.getClientRects().length&&!n.closest('.garden-header,.g-reading-tools,.g-choice-reading')&&!n.parentElement?.closest('blockquote,p,[data-readable-text]'));
  const text=[...(!s.panel?[feedback]:[]),...blocks.map(n=>{if(n.matches('button'))return n.getAttribute('aria-label')??n.textContent??'';const copy=n.cloneNode(true) as HTMLElement;copy.querySelectorAll('.g-reading-tools').forEach(tool=>tool.remove());return copy.textContent??'';})].map(t=>t.trim()).filter(Boolean);
  store.send({type:'INSPECT_TEXT',text:text.slice(0,200),focus:captureFocus(opener)});
 }
 ...
 const go=(target:'mara'|'grandma'|'sections'|'sol')=>store.send({type:'GO',point:target==='sol'?solApproach(c):target==='mara'?(maraAtGarden(c)?{x:anchors.gathering.mara.x-1.05,z:anchors.gathering.mara.z}:anchors.dock.approach):target==='grandma'?GRANDMA_APPROACH:CROSSING,target:target==='sections'?'the bridge pieces':target==='mara'?'Mara':target==='sol'?'Sol':'Grandma'});
 ...
 function paragraph(text:string,index:number,id:string){
  let wordIndex=0;const sentenceParts=text.match(/[^.!?]+[.!?]?/g)??[text];let cursor=0;
  return <div className="g-source-part" key={id+index}>{id in sources&&sourceVoice(id as SourceId,index,c)&&<p className="g-source-voice">{sourceVoice(id as SourceId,index,c)}</p>}<p data-source-component={sourcePrefix(id,c)+(index+1)} {...speechAttributes({text,origin:'authored-display',owner:c.runId+':'+readingKey,...(id in sources?{source:{id,paragraph:index,edition:editionOf(c),...(maintenanceOf(c)?{maintenanceEdition:maintenanceOf(c)!}:{}),start:0,end:text.length}}:{})})}>
   {text.split(/([A-Za-z]+(?:['’][A-Za-z]+)?)/g).map((token,i,parts)=>{
    ...
    const n=wordIndex++;let end=0;const sentence=sentenceParts.find(part=>{end+=part.length;return offset<end;})?.trim()??text;
 ...
 return <NarrativeReadingContext.Provider value={editionOf(c)}><ReadingContext.Provider value={s.panel?{word:target=>{stopAudio();setWord(target);store.send({type:'HELP_USED',id:'word:dialogue:'+normalize(target.word)});},speak,practice:practicePart,compose:(text,id,opener,onTranscript)=>{practiceFocus.current=opener;stopAudio();setReadingTarget({text,id,origin:'mixed-display',onTranscript});},stop:stopAudio,speaking}:null}><main className={'garden-app g-adaptive-reader '+(c.largeText?'garden-large ':'')+(c.largestText?'garden-largest ':'')+(c.roomierText?'garden-roomier ':'')+(c.reducedMotion?'garden-reduced-motion ':'')+(s.panel?'has-reader ':'')+(s.panel==='opening'?'reader-opening ':'')+(s.panel==='writing'?'reader-writing ':focusedStory?'reader-story ':'')+(!c.started?'in-studio':'')} role={s.panel?'dialog':undefined} aria-modal={s.panel?true:undefined} aria-labelledby={s.panel?'garden-reader-title':undefined} onKeyDown={trap} onPointerDownCapture={rememberReader} onClickCapture={rememberReader} onSelectCapture={rememberReader} onScrollCapture={rememberReader} onKeyUpCapture={rememberReader}>
  <header className="garden-header"><a href="/garden" className="garden-brand" aria-label="Garden Adventure" inert={!!s.panel} aria-hidden={!!s.panel}><span className="garden-seal"><Icon kind="leaf"/></span><span>Evidence Quest<span className="garden-brand-sub">THE GARDEN ADVENTURE</span></span></a>
   <div className="garden-header-right"><span className="g-save" aria-live="polite">{s.save==='saved'&&s.savedRevision>=c.revision?'Saved in this browser':s.save==='saving'?'Saving…':s.save==='loading'?'Opening your adventure…':''}</span>
  ...
  {localReview&&c.started&&<div className="garden-review-tools" ref={setReviewControls} hidden={!!s.panel||!!s.activity||s.mode==='mara-story'} aria-label="Local review controls"><span>{personalGrass?'Personal review':'Local review'} · artwork awaiting review</span>{personalGrass&&<PersonalReviewFullscreen/>}</div>}
  ...
  {bakeryActive&&<details name="garden-physical-controls" className="garden-alternate-controls"><summary>Bakery directions and other controls</summary><BakeryControls s={s} store={store}/></details>}
   ...
   {s.panel&&<section className={'garden-reader '+(['writing','lanterns','studio','journal','storyboard'].includes(s.panel)?'g-wide-reader':'')} ref={dialog}>
    ...
    <div className="garden-reading-scroll" inert={!!readingTarget||!!word} ref={scroll} onScroll={e=>{if(readingKey)store.send({type:'READ_POSITION',id:readingKey,position:e.currentTarget.scrollTop});}}>
     <ConversationReadingTools root={scroll}/>
     ...
     {source&&!practice&&(['story','empty','picnic','duet'].includes(source.id)||source.id==='sol'&&remoteAccount)?<StoryReadingTools chapter={c} id={source.id as 'story'|'empty'|'picnic'|'duet'|'sol'}/>:source&&!practice&&(source.id!=='sol'||conversationPending)&&<div className="g-row g-reading-actions"><button className="g-text-button" onClick={()=>speaking?stopAudio():speak(speechDocument(shownParagraphs.map(p=>({text:p.text,origin:'authored-display',owner:c.runId+':'+source.id,revision:c.revision,source:{id:source.id,paragraph:p.index,edition:editionOf(c),...(maintenanceOf(c)?{maintenanceEdition:maintenanceOf(c)!}:{}),start:0,end:p.text.length}}))))}><Icon kind="sound"/>{speaking?'Stop listening':'Hear this page'}</button><button className="g-text-button" onClick={event=>{practicePart(shownParagraphs[0]!.text,source.id+'-practice',event.currentTarget);store.send({type:'HELP_USED',id:'oral-practice:'+source.id});}}>Read a short part aloud</button></div>}
     ...
     {s.panel==='bakery'&&!remoteAccount&&!conversationPending&&conversationReady(c,'bakery')&&<BakeryConversation s={s} store={store}/>}
     ...
     {s.panel==='help'&&s.readingInspection&&<><ReadingParagraph>These are the words and choices you just saw. Reading them leaves your choices open. Return to the game to act.</ReadingParagraph>{s.readingInspection.text.map((text,index)=><ReadingParagraph origin="mixed-display" key={index}>{text}</ReadingParagraph>)}</>}
  ...
  {c.started&&!s.panel&&!s.activity&&s.mode!=='mara-story'&&<footer className="garden-action-bar">
   ...
   <div className="garden-nearby" aria-label="Nearby actions">
    {s.mode==='workbench'?<span className="g-small">Move a picture to try a different telling.</span>:gatheringActive?<span className="g-small">Follow the gathering here. Help and Pause keep your place.</span>:bakeryActive?<span className="g-small">{s.mode==='bakery-repair'?'You direct Sol’s repair':'You guide Pip · Rina and Sol do their own work'}</span>:s.mode==='boat'?<span className="g-small">You steer the boat · Pip waits on the bank</span>:s.mode==='arrange'?<span className="g-small">Drag to move · Release to place · Click a deck to walk · Escape to cancel</span>:<>
