...
import {bakeryInstruction,type BakeryStep} from './bakery.js';
import {Choices,Reply} from './Dialogue.js';
import {ReadingParagraph,ReadingTools} from './Reading.js';
...
export function BakeryConversation({s,store}:{s:GardenState;store:GardenStore}){
 ...
 // These optional notes have approved narrator recordings. Keep them neutral
 // and explicitly attributed; never substitute that voice for a Rina reply.
 const question=(id:string,title:string,paragraphs:readonly string[])=>{
  const speech=speechDocument(paragraphs.map(text=>({text,speaker:'narrator',origin:'authored-display'})));
  return <ReadingDetails id={'rina-'+id} s={s} store={store}><summary>{title}</summary><div data-narration="true" data-speech-speaker="narrator"><strong className="g-speaker" data-reading-command="true">Narrator explains</strong>{paragraphs.map(text=><ReadingParagraph key={text}>{text}</ReadingParagraph>)}</div><ReadingTools text={speech.text} speech={speech} label="this explanation"/></ReadingDetails>;
 };
 ...
 if(b.stage==='needed'||b.stage==='carried')return <>
  ...
  <p data-reading-command="true">{bakeryInstruction(s)}</p>
  {question('moving-flour','Why not just move the flour?', ['Moving the sacks keeps them dry for now, but the roof will keep leaking until the cracked tile is replaced.'])}
 </>;
 if(['delivered','gap','misplaced'].includes(b.stage))return <>
  ...
  <p data-reading-command="true">Return to the roof repair to choose and place the tile.</p>
 </>;
 if(b.stage==='sealed')return <>
  <div data-narration="true" data-speech-speaker="narrator"><ReadingParagraph>The last drops fell into the puddle. Outside, rain still tapped on the roof. Rina looked up, then put a hand on one of the flour sacks.</ReadingParagraph></div>
  ...
  {question('wet-flour','Why does the flour need to stay dry?', ['Rainwater can carry dirt through the roof.','Once we know this flour stayed dry, we can measure it and add clean water for the dough.'])}
 </>;
 if(b.stage==='checked')return <>
  <div data-narration="true" data-speech-speaker="narrator"><ReadingParagraph>{b.unshapedBatches?'Rina set the unevenly baked lump aside. The middle was still doughy. She opened another dry sack and brought a clean bowl to the counter.':'Rina opened the sacks. The flour was dry. Sol packed his tools and returned to his workshop, leaving Pip beside the mixing bowl.'}</ReadingParagraph></div>
 ...
 if(b.stage==='mixed')return <>
  <div data-narration="true" data-speech-speaker="narrator"><ReadingParagraph>Rina measured the ingredients. Pip helped mix them, and Rina pressed and folded the dough. After it had rested and risen, one large lump filled the bowl.</ReadingParagraph></div>
 ...
 if(b.stage==='shaped')return <>
  <div data-narration="true" data-speech-speaker="narrator"><ReadingParagraph>Pip helped divide the dough. Rina shaped the pieces into three loaves and arranged them with space between them.</ReadingParagraph></div>
  ...
  {question('repair-mattered','What would have happened without the repair?', ['Rainwater can carry dirt through the roof.','The repaired roof kept the flour dry. Rina used it to bake the bread she had promised.'])}
 </>;
 if(b.stage==='baked')return <>
  <div data-narration="true" data-speech-speaker="narrator"><ReadingParagraph>A little later, Rina lifted the golden loaves from the oven. She set them aside to cool, then chose one for Sol.</ReadingParagraph></div>
  ...
  {question('orders','What about the people waiting for their bread?', ['Rina takes one baked loaf to thank Sol. The other bread stays for the people she promised.'])}
 </>;
 if(b.stage==='escorting')return <><Reply who="Rina">I have Sol’s loaf. Walk with me along the path to his workshop. If you need to look at something, I’ll wait for you.</Reply><p data-reading-command="true">Return to the game and walk with Rina. The loaf stays in her hands until she gives it to Sol.</p></>;
 if(b.stage==='done')return <>
  ...
  <div data-narration="true" data-speech-speaker="narrator"><ReadingParagraph>Sol looked from the loaf to the page on his workbench. He had started writing about the repair, but his draft stopped before the baking and Rina’s visit.</ReadingParagraph></div>
 ...
 return <p data-reading-command="true">{bakeryInstruction(s)}</p>;
}
