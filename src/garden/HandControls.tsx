...
export function HandControls({s,store}:{s:GardenState;store:GardenStore}){
 ...
 if(s.panel||s.activity||s.mode==='workbench'||(!objects.length&&!gesture))return null;
 ...
 return <aside className="garden-hands" aria-label="Handle nearby objects">
  <p>{planting?plantingGuidance(s.chapter):gesture?handDefinitions[gesture.object].instruction:'Drag an object, then release to place it. Click a deck or the ground to walk. You can leave a repair unfinished.'}</p>
