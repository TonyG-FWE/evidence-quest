...
export function StoryReadingTools({chapter,id,version='available'}:{chapter:Chapter;id:CompleteStoryId;version?:StoryReadingVersion}){
 ...
 return <section className="g-story-reading" aria-label={'Read the complete '+story.title}>
  ...
  <div className="g-row"><button className="g-secondary" onClick={()=>support.speak(story.speech??{text:story.text,origin:story.origin})}>Hear the whole story</button><button className="g-secondary" onClick={event=>support.practice(story.text,'whole-story:'+id,event.currentTarget,story.origin,story.title,story.speech)}>Read the whole story aloud</button>{support.speaking&&<button className="g-text-button" onClick={support.stop}>Stop listening</button>}</div>
 </section>;
}
