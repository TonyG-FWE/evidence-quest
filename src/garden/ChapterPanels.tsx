...
import {preparedEndingsFor,sourcesFor,endingLinesFor} from './content.js';
import {AdaptiveWriting} from './ReaderPresentation.js';
import type {SpeechSpeak} from './voiceTypes.js';
...
type Props={s:GardenState;store:GardenStore;open:(p:Panel,origin?:Element)=>void;renderSource:(id:SourceId,index?:number)=>ReactNode;renderText:(line:SpokenLine)=>ReactNode;speak:SpeechSpeak;practice:(text:string,id:string,opener?:HTMLElement,origin?:ReadingOrigin)=>void};
export function ChapterContent(props:Props){
 const {s,store,open,renderSource}=props,c=s.chapter,f=c.story,endingLines=endingLinesFor(c);
 ...
 if(s.panel==='mara'&&f.phase==='planning'&&conversationReady(c,'mara')&&c.mara.asking==='pending')return <ReadingDetails id="mara-later-reflection" s={s} store={store}><summary>Think about a later gathering</summary><FeedbackActivity store={store} activity="later-time" speaker="Mara" question="How would starting later help me get to the gathering?" help="Mara has to help the last passengers ashore. Starting after the boat returns gives her time to finish that work and come to Grandma’s garden. Pip still needs to ask Grandma and tell everyone the agreed plan." sourceIds={['mara']}/></ReadingDetails>;
 ...
 if(s.panel==='grandma'){
  ...
  return <>
   ...
   {c.seed!=='soil'&&<ReadingParagraph>{c.seed==='bed'?'The seed is in the prepared spot. Cover it with Grandma.':c.seed==='grandma'?'Grandma has the seed from the seed boat. Pip is here now, so they can plant it together.':'Pip is carrying the seed in his backpack. Prepare the spot, place the seed and cover it together.'}</ReadingParagraph>}
   ...
   <Choices label="What will Pip do next?">
    {c.seed!=='soil'&&<button className="g-primary" disabled={!!s.action} onClick={()=>{store.send({type:'BACK'});store.send({type:'NOTICE',text:'Work beside Grandma: draw through the soil, place the seed, then cover it. Keyboard controls handle the same objects.'});}}>Work beside Grandma</button>}
...
function Writing({s,store,renderSource,speak,practice}:Props){
 ...
 return <AdaptiveWriting store={store} source={renderSource('sol')} later={renderSource('later')} onHear={part=>speak(speechDocument(chapterSources[part==='draft'?'sol':'later'].paragraphs.map((text,paragraph)=>({text,origin:'authored-display',source:{id:part==='draft'?'sol':'later',paragraph,edition:editionOf(s.chapter),...(maintenanceOf(s.chapter)?{maintenanceEdition:maintenanceOf(s.chapter)!}:{}),start:0,end:text.length}}))))} action={<button className="g-primary" disabled={s.save==='conflict'||!f.solDraft.text.trim()} onClick={e=>store.send({type:'REHEARSE',contribution:{text:f.solDraft.text,scene,revision:f.solDraft.revision,origin:'child'},focus:captureFocus(e.currentTarget)})}>Try my ending</button>} draft={<>
  ...
  <textarea id="sol-ending" readOnly={s.save==='conflict'} value={f.solDraft.text} onChange={e=>send({kind:'EDIT',text:e.target.value})} placeholder="Write your ending here…" rows={6}/>
  <DraftReader store={store} text={f.solDraft.text} revision={f.solDraft.revision} speak={speak} practice={practice}/>
  <Choices label="Choose the part your picture will show.">{(['bread','thanks','both'] as const).map(v=><button key={v} className="g-secondary" aria-pressed={scene===v} disabled={s.save==='conflict'} onClick={()=>setScene(v)}>{v==='bread'?'Rina bakes the bread':v==='thanks'?'Rina thanks Sol':'Baking, then the visit'}</button>)}</Choices>
  <ReadingDetails id="ending-help" s={s} store={store}><summary>Support for my ending</summary><FeedbackActivity store={store} activity="sol-ending" speaker="Sol" question="Would you like help with your ending?" help="Fixing the tile kept the flour dry. Rina could bake the bread she had promised. Later, she brought Sol a loaf to thank him. You can choose which part to show." sourceIds={['sol','later']} externalText={f.solDraft.text} onExternalChange={text=>send({kind:'EDIT',text})} onScene={setScene}/></ReadingDetails>
  <ReadingDetails id="prepared-endings" s={s} store={store}><summary>Help me start with a prepared ending</summary><ReadingParagraph>These are prepared words. Your own draft stays in its writing area.</ReadingParagraph>{(['bread','thanks'] as const).map(v=><div key={v}>{renderSource(v==='bread'?'breadEnding':'thanksEnding')}<button className="g-secondary" disabled={s.save==='conflict'} onClick={e=>store.send({type:'REHEARSE',contribution:{text:preparedEndings[v],scene:v,revision:f.solDraft.revision,origin:'prepared'},focus:captureFocus(e.currentTarget)})}>Try the {v==='bread'?'bread':'thank-you'} ending</button></div>)}</ReadingDetails>
  {f.solEnding&&<div className="g-selected-ending"><h3>Sol’s chosen ending</h3><ReadingParagraph origin={f.solEnding.origin==='child'?'child-draft':'authored-display'} speech={solContributionSpeech(s.chapter,f.solEnding)}>{f.solEnding.text}</ReadingParagraph><ReadingParagraph>{f.solEnding.origin==='prepared'?'Prepared ending from Sol’s account.':'Your suggested ending. These words have not been checked against Sol’s account.'} Sol will share the version you last chose.</ReadingParagraph></div>}
 </>}/>;
}
...
function Gathering({s,store,open,renderSource,speak,practice}:Props){
 ...
 if(f.phase==='welcome')return <><Reply who="Grandma">You helped arrange today's gathering, Pip. Would you like to welcome everyone?</Reply><h3>Pip’s welcome</h3><ReadingParagraph>{welcome(c)}</ReadingParagraph><button className="g-secondary" onClick={()=>speak({text:welcome(c),speaker:'pip',origin:'authored-display'})}>Hear the welcome</button><button className="g-secondary" onClick={event=>practice(welcome(c),'welcome-'+f.plan!.revision,event.currentTarget)}>Practise the welcome</button><button className="g-primary" onClick={()=>send({kind:'NEXT_STORY'})}>Start the first story</button></>;
 ...
 if(f.phase==='sol')return <><Reply who="Pip">{f.solChoice==='prepared'?"Sol and I worked on the ending of his story. He's ready to share it.":"Sol has brought a draft of his story. After he reads it, we can tell him what we'd like to hear more about."}</Reply><StoryReadingTools chapter={c} id="sol"/><h3>A Small Repair · by Sol</h3>{renderSource('sol')}{f.solChoice==='prepared'&&<><StoryStage compact scene={f.solEnding!.scene}/><h3>{f.solEnding!.origin==='child'?'Sol reads your suggested ending':'The prepared ending Sol chose'}</h3>{f.solEnding!.origin==='child'&&<ReadingParagraph>The picture shows the event you chose. Your suggested words have not been checked against Sol’s account.</ReadingParagraph>}{f.solEnding!.origin==='prepared'?renderSource(f.solEnding!.scene==='thanks'?'thanksEnding':'breadEnding'):<ReadingParagraph className="g-manuscript" origin="child-draft" speech={solContributionSpeech(c,f.solEnding!)}>{f.solEnding!.text}</ReadingParagraph>}<button className="g-secondary" onClick={()=>speak(solContributionSpeech(c,f.solEnding!))}>Hear the ending</button></>}<button className="g-primary" onClick={()=>send({kind:'NEXT_STORY'})}>{f.solChoice==='prepared'?'Listen to Grandma’s story':'Ask Sol about his draft'}</button></>;
 if(f.phase==='discussion')return <>
  ...
  {ending&&<div className="g-rehearsal"><h3>Try this ending</h3><StoryStage compact scene={ending} animate/>{renderSource(ending==='bread'?'breadEnding':'thanksEnding')}</div>}
