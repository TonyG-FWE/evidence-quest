...
export function WorldActivityView({s,store,open,speak,audio,speaking,stopAudio}:{s:GardenState;store:GardenStore;open:(panel:Panel,origin?:Element)=>void;speak:(text:string)=>void;audio:string;speaking:boolean;stopAudio:()=>void}){
 ...
 useEffect(()=>{
  ...
  const tick=(now:number)=>{const current=store.getSnapshot(),delta=now-last;last=now;
   if(current.activity&&(current.activity.elapsed<6000||current.playback&&!current.playback.paused&&current.playback.mode==='captions')&&!current.panel&&!current.background&&!current.viewLost)store.send({type:'TICK',ms:delta});
 ...
 return <section className="garden-world-activity" aria-labelledby="world-activity-title" data-world-activity={activity.kind}>
  ...
  <StoryStage scene={scene} animate time={s.chapter.reducedMotion?6:activity.elapsed/1000}/>
  <div className="g-world-activity-caption">{ending&&<><h3>{activity.contribution.origin==='child'?'Your suggested ending for Sol':'Prepared ending from Sol’s account'}</h3><p className="g-small">{activity.contribution.origin==='child'?'The picture shows the event you chose. Feedback is unavailable, so these words have not been checked against Sol’s account. They will stay labeled as your suggested ending.':'These prepared words come from the events in Sol’s account.'}</p></>}<p className={ending?'g-manuscript':''}>{text}</p><p className="g-small">{ending?'Choose Use this ending to select these exact words, or return to your draft.':'Choose Use this plan to agree it with Grandma, or return to change it.'}</p>
