...
/** Inspect a durable checkpoint while travel continues. This deliberately does
 * not wait for the UI to acknowledge the latest moving position. */
export async function readPersistedChapter(page:Page){
 return page.evaluate(async()=>new Promise<any>((resolve,reject)=>{
  ...
  request.onsuccess=()=>{const db=request.result;if(!db.objectStoreNames.contains('slots')){db.close();resolve(undefined);return;}
   const transaction=db.transaction('slots'),read=transaction.objectStore('slots').get('current');transaction.onerror=()=>{db.close();reject(transaction.error);};transaction.onabort=()=>{db.close();reject(transaction.error);};transaction.oncomplete=()=>{db.close();resolve(read.result?.payload);};
  };
 }));
}
...
export async function savedChapter(page:Page){
 ...
 await expect.poll(async()=>{
  payload=await readPersistedChapter(page);return !!payload;
