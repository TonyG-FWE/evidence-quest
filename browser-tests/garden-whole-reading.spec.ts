...
test('Complete reading: the whole story is heard in order, can stop, and resumes without sending the page',async({page},info)=>{
 ...
 const tools=page.getByRole('region',{name:'Read the complete The Torn Wing',exact:true});await tools.getByRole('button',{name:'Hear the whole story',exact:true}).click();
