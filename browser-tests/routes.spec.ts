...
test('TASK11.16 correct-first, in-place search plan and Loop-first independent recovery',async({page})=>{
 ...
 await go(page,'WK.EXIT.MD','Media room');await target(page,'ACT.LOOP');await settled(page);await stage(page);await target(page,'ST.DOCK');await settled(page);await target(page,'ST.RAIL');await settled(page);
 await expect(page.getByText('Bring the story kit to the Stage desk so you can plan the ending.')).toBeVisible();expect((await saved(page)).payload.physical.caddyHost).toBe('MD.RACK.STATION');
...
test('TASK11.16 cancellation evidence delivered to Remy, kit-first recovery and seed-only revision',async({page})=>{
 ...
 await go(page,'CY.EXIT.WK','Workshop');await go(page,'WK.EXIT.MD','Media room');await collectKit(page);await stage(page);await target(page,'ST.RACK.BAY');await settled(page);
 ...
 await page.getByRole('button',{name:'Try this ending',exact:true}).click();await expect(page.getByText('Bring Loop to the dock to project the story.').first()).toBeVisible();
 ...
 expect((await saved(page)).payload.physical.order).toEqual(['TILE.FERRY','TILE.PLANT','TILE.BLOOM']);await page.getByRole('button',{name:'Try this ending',exact:true}).click();await expect(page.getByRole('button',{name:'Continue rehearsal',exact:true})).toBeVisible();const failed=await saved(page);expect(failed.payload.playback?.puppet).toEqual({pip:'left',seed:'right',boats:'separate',lit:false});expect(failed.payload.certificate).toBeNull();
