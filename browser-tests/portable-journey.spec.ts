...
test('portable offline journey reaches the finale and six-story library using real recorded voices',async({page},info)=>{
 ...
 // Returning from the ending reloads the village assets asynchronously.
 await expect(page.locator('.garden-scene')).toHaveAttribute('data-scene-phase','ready',{timeout:90000});
 await expect.poll(async()=>JSON.parse(await page.locator('.garden-scene').getAttribute('data-character-assets')??'{}').pending,{timeout:90000}).toBe(0);
 await page.screenshot({path:info.outputPath('complete-finale-ready.png')});
