...
for(const edition of ['original','literary-20260916'] as const)test('Narrative help uses the actual '+edition+' words in display, listening and oral practice',async({page},info)=>{
 ...
 await expect(help.locator('[data-readable-text]')).toHaveText(expected);await help.getByRole('button',{name:'Hear Sol’s reply',exact:true}).click();await expect.poll(async()=>(await spokenWords(page)).join(' ').replace(/\s+/g,' ').trim()).toBe(expected.replace(/\s+/g,' ').trim());
 await help.getByRole('button',{name:'Read Sol’s reply aloud',exact:true}).click();await expect(page.locator('.g-practice-words')).toHaveText(expected);await expect(page.locator('.g-practice-overlay')).toHaveAttribute('data-recording','ready');
