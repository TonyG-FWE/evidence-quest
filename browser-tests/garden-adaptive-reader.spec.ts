...
test('actual writing: source panes, equal choices, durable draft and Try versus Use',async({page},info)=>{
 ...
 const before=await savedChapter(page);await button(page,'Hear choice: Baking, then the visit').click();expect((await savedChapter(page)).story.solChoice).toBe(before.story.solChoice);expect((await savedChapter(page)).story.solEnding).toEqual(before.story.solEnding);
...
test('synthetic late microphone permission after native focus loss never starts capture',async({page})=>{
 ...
 await page.goto('/garden');await button(page,'Begin Pip’s adventure').click();await button(page,'Read a short part aloud').click();await button(page,'Start listening').click();
...
test('editing an answer aborts its pending selected-voice request before stale audio arrives',async({page})=>{
 ...
 await writing(page);const draft=page.getByLabel('Your ending for Sol’s story',{exact:true});await draft.fill('I repaired the roof so Rina could bake.');await page.getByText('Support for my ending',{exact:true}).click();await button(page,'Ask for feedback').click();await expect(button(page,'Hear the feedback')).toBeVisible();
 const request=page.waitForRequest('**/api/garden/cast-audio');await button(page,'Hear the feedback').click();await request;
 await draft.fill('I repaired the roof. Later Rina brought a loaf.');await expect.poll(()=>page.evaluate(()=>(window as any).staleVoiceAborts)).toBeGreaterThan(0);release();await expect(button(page,'Hear the feedback')).toHaveCount(0);await expect(draft).toHaveValue('I repaired the roof. Later Rina brought a loaf.');
});
...
test('saved child endings keep Sol voice and exact revision in library and completed-story word help',async({page})=>{
 ...
 for(const location of ['library','story'] as const){
  ...
  await words.locator('.g-word').first().click();const count=requests.length;await button(page,'Hear the sentence').click();await expect.poll(()=>requests.length).toBe(count+1);
...
test('long personal notes send a compact exact-text revision only after explicit listening',async({page})=>{
 ...
 await button(page,'Hear my notebook notes').click();await expect.poll(()=>revisions.length).toBe(1);
