...
export async function readWholeStory(page:Page,title:string,expected:string){
 ...
 await tools.getByRole('button',{name:'Practise reading the whole story',exact:true}).click();
 ...
 await expect(tools.getByRole('button',{name:'Practise reading the whole story',exact:true})).toBeFocused();
}
