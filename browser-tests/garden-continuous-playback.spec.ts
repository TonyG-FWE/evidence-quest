...
test('Continuous telling: complete Sol words, safe word help, replay and stale callback ownership',async({page},info)=>{
 ...
 await button(page,'Replay story').click();await expect.poll(async()=>(await saved(page)).gathering.turn.index).toBe(0);await page.evaluate(()=>{const voice=(window as any).__playbackVoice;voice.spoken=[voice.pending.text];});await playSpeakingTurn(page);
 expect((await spokenWords(page)).join('\n\n')).toBe(expected.join('\n\n'));const after=await saved(page);expect(after.gathering.solPerformed).toEqual(before.gathering.turn.contribution);expect(after.gathering.turn).toBeNull();for(const key of ['seed','river','bakery','page','pip'])expect(after[key]).toEqual(before[key]);
});

test('Continuous telling: restored progress waits, full Grandma telling proceeds without sentence choices',async({page},info)=>{
 await installPlaybackVoice(page);const original=await restore(page,'grandma-original-first-paragraph');await expect(button(page,'Resume story')).toBeVisible();expect(await spokenWords(page)).toEqual([]);
 ...
 await button(page,'Replay story').click();const replay=await saved(page),expected=turnLines(replay).map(line=>line.text);await playSpeakingTurn(page);expect((await spokenWords(page)).join('\n\n')).toBe(expected.join('\n\n'));expect((await saved(page)).gathering.grandmaPerformed).toBe(true);await info.attach('scope',{body:'Real save restoration and visible resume/replay. Synthetic local speech delivers every exact canonical paragraph and subsequent replies. No per-sentence action or inferred reading assessment.',contentType:'text/plain'});
});

test('Continuous Loop: full ending, automatic pictures, pause and reload keep the actual story',async({page},info)=>{
 await installPlaybackVoice(page);await restore(page,'later-pip-prepared-before-pip-moment');await page.locator('.garden-access>summary').click();await page.locator('.garden-access').getByRole('button',{name:'Go to Grandma',exact:true}).click();await page.locator('.garden-access>summary').click();await button(page,'Talk to Grandma E').click();await button(page,'Continue the gathering').click();await button(page,'Sharing our stories').click();await button(page,'Keep this moment').click();await button(page,'Hear Grandma and Mara finish their conversation').click();await playSpeakingTurn(page);await button(page,'Finish the chapter').click();
 ...
 await finishVoiceSegment(page);await expect.poll(async()=>(await saved(page)).story.presentation.page).toBe(1);await button(page,'Pause story').click();const held=await saved(page);await page.evaluate(()=>(window as any).__playbackVoice.callbacks.at(-1)?.());expect((await saved(page)).story.presentation.page).toBe(held.story.presentation.page);await button(page,'Pause · Back to studio').click();await page.reload();await button(page,'Watch your ending').click();await button(page,'Watch the ending').click();await expect(page.locator('[data-presentation-page="1"]')).toBeVisible();await button(page,'Replay story').click();await page.evaluate(()=>{const voice=(window as any).__playbackVoice;voice.spoken=[voice.pending.text];});await playLoopStory(page);
 const words=(await spokenWords(page)).join('\n\n');expect(words).toBe(completed.story.ending.paragraphs.join('\n\n'));const final=await saved(page);expect(final.story.ending).toEqual(completed.story.ending);expect(final.story.presentation).toMatchObject({finished:true,inProgress:false,page:3});await info.attach('scope',{body:'Authentic earlier checkpoint and ordinary story/Loop controls. Synthetic voice checks all four actual ending paragraphs, automatic scene progress and late-callback cancellation; no native acoustic qualification.',contentType:'text/plain'});
});

test('Continuous captions: unavailable narration advances automatically and the complete story remains readable',async({page},info)=>{
 ...
 const before=await restore(page,'sol-world-introduction');await button(page,'Resume story').click();await expect(page.getByRole('group',{name:'Whole story playback',exact:true})).toHaveAttribute('data-playback-mode','captions');
