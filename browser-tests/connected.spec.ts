...
test('TASK11.16 media-first complete case, notes stay unread through collection, real rehearsal and premiere',async({page})=>{
 ...
 await go(page,'ST.EXIT.WK','Workshop');await go(page,'WK.EXIT.MD','Media room');
 ...
 await target(page,'ST.RACK.BAY');await expect(page.getByRole('heading',{name:'Plan our story'})).toBeVisible();const seated=await settled(page);expect(seated.payload.physical.caddyHost).toBe('ST.RACK.BAY');expect(seated.payload.grants.some(g=>['E6','E7'].includes(g.sourceId))).toBe(false);
 ...
 await page.getByRole('button',{name:'Try this ending',exact:true}).click();await expect(page.getByRole('button',{name:'Start premiere',exact:true})).toBeVisible();const certified=await saved(page);expect(certified.payload.certificate).not.toBeNull();expect(certified.payload.premiere).toBeNull();
 await page.screenshot({path:'output/playwright/rehearsed-first-case.png',fullPage:true});
 await page.getByRole('button',{name:'Start premiere',exact:true}).click();await expect(page.getByRole('heading',{name:'The premiere',exact:true})).toBeVisible();const completed=await saved(page);expect(completed.payload.premiere?.order).toEqual(['TILE.BRIDGE','TILE.PLANT','TILE.BLOOM']);expect(completed.payload.historyUncertain).toBe(false);expect(errors).toEqual([]);
