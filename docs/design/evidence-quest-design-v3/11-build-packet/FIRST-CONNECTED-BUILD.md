...
**Future implementation instructions. Nothing in this file has been executed.** Read [the entry point](../11-FINAL-CODEX-BUILD-PACKET-AND-REVISED-SCHEDULE.md), and the completed [Item 12 checkpoint](../12-BUILD-READINESS-CHECKPOINT.md). Readiness is established; a subsequent implementation-scoped user instruction is still required. This document defines **M11.CONNECTED**, not the illustrated release or live-AI qualification.
...
## Time assumptions and calibration

Current Codex elapsed time is **uncalibrated**. The old 69–114 hours including readiness and 68–112 after it are historical mixed allowances, not measured AI execution or human coding time. TASK11.00 is now documented complete; its old 1–2 hours is not an actual duration. No deadline fit/miss follows from those numbers.

During authorized TASK11.01–02, 04–06 and 16 record actual AI iteration time, human attention, temporary-generation/preparation, integration/testing/repair, waits and any disjoint overlap. Update the same implementation-plan.json schedule and regenerate its view from measured comparable work; do not infer final illustrated-art throughput from temporary graphics. See [the corrected schedule](REVISED-SCHEDULE.md). No calibration work runs as part of this review.
...
Core fixture set: FIX11.OPEN through FIX11.RECAP, excluding FIX11.TOOLS, explicitly including SAVE_RECOVERY, SAVE_INTERRUPTED, RESET, ART_FAILURE and RECAP; FIX11.TOOLCHAIN/CONTENT/TEMP; authored/direct/draft and transport-fault coaching fixtures; FIX11.ORDER.00–64. Where a fixture has later final-art/live/participant criteria, pass only its applicable first-build part and leave the rest NOT_RUN. Do not mark the entire fixture passed because its local subset works.

## Complete future execution prompt

Item 12 has established this scope. Sending the following as a new implementation instruction can authorize it; merely reading or reviewing this file does not.
...
>
> Record actual AI iteration elapsed time, human attention, preparation/export time, test/repair cycles, waits and disjoint overlap during the existing calibration tasks in the plan. Reforecast from those observations; do not reuse the historical 69–114/68–112 or 190–314-hour allowances as AI forecasts.
>
