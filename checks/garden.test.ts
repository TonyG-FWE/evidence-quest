...
import {sources,words,meaning} from '../src/garden/content.js';
...
function tick(store:GardenStore,ms=8000){for(let i=0;i<ms/80;i++)store.send({type:'TICK',ms:80});}
function go(store:GardenStore,point:Point){store.send({type:'GO',point});tick(store,12000);assert.ok(Math.hypot(store.getSnapshot().chapter.pip.x-point.x,store.getSnapshot().chapter.pip.z-point.z)<.01,'Actor must actually reach the target');}
...
function secure(store:GardenStore){store.send({type:'FASTEN',end:'west'});store.send({type:'FASTEN',end:'east'});assert.equal(bridgeReady(store.getSnapshot().chapter),true);store.send({type:'BACK'});}
function ferry(s:GardenStore){s.send({type:'FERRY'});tick(s);if(s.getSnapshot().chapter.seed==='boat'){s.send({type:'LAUNCH_BOAT'});for(const point of [{x:-1.4,z:0},{x:1.4,z:0},{x:1.4,z:.8}]){s.send({type:'STEER',point});tick(s);}s.send({type:'DOCK_SEED'});tick(s);}}
...
test('D029 / D081.R15.AC01-03: carried seed, actual walking, page report and source remain distinct',()=>{
 ...
 cross(s);s.send({type:'TALK',who:'grandma'});s.send({type:'PLANT'});tick(s);
...
test('Boat/bridge correction: seed boat never moves bridge sections, Pip or his page',()=>{
 ...
 assembly(s);secure(s);cross(s);s.send({type:'PLANT'});tick(s);assert.equal(s.getSnapshot().chapter.seed,'soil');
});
...
test('D030 / D081.R15.AC02: visible wide gap, wrong anchor, loose end and direct correction',()=>{
 const s=game();assembly(s,-2,-.8);s.send({type:'FASTEN',end:'west'});s.send({type:'FASTEN',end:'east'});assert.equal(s.getSnapshot().notice,GAP);assert.equal(bridgeReady(s.getSnapshot().chapter),false);
 s.send({type:'ADJUST'});const middle=bridgeCenter(s.getSnapshot().chapter);section(s,'a',{x:s.getSnapshot().chapter.sections.a.x-middle.x,z:3});s.send({type:'FASTEN',end:'west',atZ:-2});assert.equal(s.getSnapshot().chapter.west,false);
 s.send({type:'FASTEN',end:'west',atZ:3});assert.equal(s.getSnapshot().notice,LOOSE);s.send({type:'BACK'});s.send({type:'GO',point:GRANDMA_APPROACH});tick(s);assert.ok(s.getSnapshot().chapter.pip.x<0);
 assert.equal(s.getSnapshot().chapter.story.bridgeFailures,1);assert.equal(s.getSnapshot().chapter.joined,false);assembly(s);secure(s);cross(s);
});
test('D081.R15.AC04: five equal physical histories, including F between planting/growth and trailing F',()=>{
 for(const history of ['BPL','FBPL','BFPL','BPFL','BPLF']){
  ...
  for(const cue of history){
   ...
   if(cue==='F')ferry(s);
   if(cue==='P'){s.send({type:'PLANT'});s.send({type:'INTERRUPT'});s.send({type:'CLOSE'});}
...
test('D081.R15.AC05: cancellation, blur and duplicate actions preserve one committed result',()=>{
 ...
 s.send({type:'BACK'});s.send({type:'FERRY'});s.send({type:'FERRY'});s.send({type:'INTERRUPT',background:true});tick(s);assert.equal(s.getSnapshot().chapter.seed,'boat');assert.equal(s.getSnapshot().chapter.history.filter(h=>h==='F').length,0);
 s.send({type:'KEY',key:'w',down:true});tick(s);assert.deepEqual(s.getSnapshot().keys,[]);
});
test('D081.R15: scenery shares physical bounds; keyboard placement is reversible until committed',()=>{
 const s=game(),c=s.getSnapshot().chapter;for(const [x,z]of TREE_POSITIONS)assert.equal(walkable(c,{x,z}),false);assert.equal(walkable(c,{x:-5.1,z:-4.5}),false);assert.equal(walkable(c,{x:5.15,z:2.28}),false);assert.equal(walkable(c,GRANDMA_APPROACH),true);
...
test('D081.R16: modal/reading input cannot move Pip and hidden sources cannot be opened',()=>{
 const s=game(),before={...s.getSnapshot().chapter.pip};s.send({type:'OPEN',panel:'opening'});s.send({type:'KEY',key:'d',down:true});tick(s);assert.deepEqual(s.getSnapshot().chapter.pip,before);
 s.send({type:'EXPOSE',id:'GA.SRC.STORY.1'});assert.deepEqual(s.getSnapshot().chapter.exposed,[]);s.send({type:'OPEN',panel:'story'});assert.equal(s.getSnapshot().panel,'opening');
 s.send({type:'EXPOSE',id:'GA.SRC.OPENING.R20260915.1'});assert.deepEqual(s.getSnapshot().chapter.exposed,['GA.SRC.OPENING.R20260915.1']);
});
...
test('Boat/bridge correction: completed old carried and ferried routes preserve their footbridge and outcomes',()=>{
 for(const ferryFirst of [false,true]){
  const s=game();receivePage(s);go(s,CROSSING);if(ferryFirst)ferry(s);assembly(s);secure(s);cross(s);s.send({type:'PLANT'});tick(s);s.send({type:'TALK',who:'grandma'});s.send({type:'REPORT'});tick(s);
