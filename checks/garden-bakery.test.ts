...
import {BAKERY_APPROACH,TILE_APPROACH,bakeryReady} from '../src/garden/bakery.js';
...
function game(){const s=new GardenStore(initialGarden('bakery-'+serial++),()=>String(serial++));s.send({type:'BOOT'});s.send({type:'BEGIN'});s.send({type:'START_PLAY'});go(s,CROSSING);buildBridge(s);go(s,GRANDMA_APPROACH);return s;}
