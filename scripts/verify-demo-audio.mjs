...
export async function verifyDemoAudio(publicRoot='public',sourceRoot=process.cwd(),{scope='source'}={}){
 ...
 for(const entry of manifest.entries){
  ...
  if(scope==='runtime'){
   ...
   const bytes=await audio(clip.uri,clip.sha256,clip.bytes);if(bytes)runtimePcm.set(clip.uri,pcmWave(bytes));const pcm=runtimePcm.get(clip.uri);if(!pcm||!Number.isSafeInteger(clip.startSample)||!Number.isSafeInteger(clip.endSample)||clip.startSample<0||pcm.sampleRate!==clip.sampleRate||pcm.channels!==clip.channels||pcm.samples!==clip.endSample-clip.startSample||Math.abs(clip.duration-pcm.samples/pcm.sampleRate)>1e-9)throw Error('Runtime PCM bounds mismatch');continue;
  }
  ...
  if(receipt.streamAssembly!=='all-audio-deltas-v2'||receipt.sha256!==entry.sha256||receipt.voiceId!==entry.voiceId||receipt.speaker!==entry.speaker||entry.binding.textSha256!==sha256(entry.text)||!matchingWords(entry.text,receipt.alignment).some(match=>match.wordStart===entry.binding.wordStart&&match.wordEnd===entry.binding.wordEnd&&match.start===entry.start&&match.end===entry.end))throw Error('Authored words do not match recorded words: '+entry.id);
 ...
 for(const entries of groups.values()){
  ...
  for(const entry of entries){
   ...
   if(clip.sourceSha256!==entry.sha256||clip.sourceWavSha256!==entry.wavSha256||entry.wavSha256!==first.wavSha256||entry.wavBytes!==master.length||clip.sampleRate!==pcm.sampleRate||clip.channels!==pcm.channels||clip.startSample!==Math.floor(entry.start*pcm.sampleRate)||clip.endSample!==Math.ceil(entry.end*pcm.sampleRate)||Math.abs(clip.duration-(clip.endSample-clip.startSample)/pcm.sampleRate)>1e-9)throw Error('Independent PCM provenance mismatch: '+entry.id);
   if(!/^\/audio\/cast\/clips\/[a-f0-9]{64}\.wav$/.test(clip.uri))throw Error('Audio clip is not independent');
   if(!checkedClips.has(clip.sha256)){const derived=slicePcmWave(master,clip.startSample,clip.endSample);if(sha256(derived)!==clip.sha256||derived.length!==clip.bytes)throw Error('Independent samples differ from master: '+entry.id);await audio(clip.uri,clip.sha256,clip.bytes);checkedClips.add(clip.sha256);}
  }
 }
