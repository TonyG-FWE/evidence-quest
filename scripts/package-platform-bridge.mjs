/** Package the six-board platform selected in Tony's annotated screenshot. */
...
const directory='evidence/final-demo-20260918',cache='.cache/final-demo-review/assets',id='platform-bridge-fitted';
const sha=value=>createHash('sha256').update(value).digest('hex');
const report=JSON.parse(await fs.readFile(directory+'/review-assets.json','utf8'));
const proofPath='output/screenshot-repairs-20260918/platform-bridge/provenance.json',proof=JSON.parse(await fs.readFile(proofPath,'utf8'));
...
const document=await io.readBinary(working),prior=report.assets.platform,runtime=await io.read(cache+'/'+path.basename(prior.uri));
if(prior.sourceSha256!==proof.sourceSha256)throw Error('Working copy does not match the selected original platform');
...
report.assets[id]={...structuredClone(prior),id,uri:receipt.uri,sha256:hash,sourceSha256:proof.outputSha256,resources:receipt,locations:['crossing'],components:{assembly:'Two independent halves from the same supplied six-board platform selected in the annotated screenshot; source component transforms fit planks and post sockets',workingCopyProvenance:proofPath,sharedCenterSockets:['socket-right-negative','socket-left-negative']}};
report.bindings=report.bindings.filter(binding=>binding.id!==id);report.bindings.push({id,source:proof.output,sourceSha256:proof.outputSha256,originalSource:proof.source,originalSourceSha256:proof.sourceSha256,triangles:proof.outputTriangles,normalizationBasis:'Unchanged original platform frame',runtimeMembership:['crossing'],selection:'SELECTED_LOCAL_REVIEW',reviewStatus:'PENDING_FORM_REVIEW'});
report.locationMembership.crossing=report.locationMembership.crossing.filter(value=>!['bridge','platform','bridge-fitted',id].includes(value));report.locationMembership.crossing.push(id);
...
await fs.writeFile(directory+'/screenshot-repair/platform-bridge-runtime.json',JSON.stringify(receipt,null,2)+'\n');console.log(JSON.stringify(receipt));
