...
const friendly=text=>/[A-Za-z]/.test(text)&&(/\s/.test(text)||/^[A-Z][a-z]+[.!?]?$/.test(text))&&!/[{}<>\[\]]|\bdata-[a-z-]+|(?:^|\s)g-[a-z-]+|(?:^|\s)(?:import|export|const)\s|\.(?:glb|png|webp|json|ts|mp3)|(?:https?:|\/api\/)|#[0-9a-f]{6}/i.test(text)&&text.length<1800;
