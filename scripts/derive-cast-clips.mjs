...
for(const wavHash of new Set(manifest.entries.map(entry=>entry.wavSha256).filter(Boolean))){
 ...
 for(const entry of manifest.entries.filter(entry=>entry.wavSha256===wavHash)){
  ...
  const whole=isWholeWordRecording(receipt,entry.text);
  const bound=whole?binding.wordStart===0&&binding.wordEnd===1&&entry.start===0&&entry.end===pcm.samples/pcm.sampleRate:matchingWords(entry.text,receipt.alignment).some(m=>m.wordStart===binding.wordStart&&m.wordEnd===binding.wordEnd&&m.start===entry.start&&m.end===entry.end);
  if(receipt.sha256!==entry.sha256||receipt.voiceId!==entry.voiceId||sha(entry.text)!==binding.textSha256||!bound)throw Error('Exact recording words do not match clip: '+entry.id);
  if(isPronunciationEntry(entry)&&!isWholeWordRecording(receipt,entry.text))throw Error('Refusing sentence-derived pronunciation: '+entry.text);
  const window=isWholeWordRecording(receipt,entry.text)?completeWordWindow(master,receipt,entry.text):/^[A-Za-z]+(?:['’][A-Za-z]+)?$/.test(entry.text)?wordClipWindow(master,receipt.alignment,binding):{startSample:Math.floor(entry.start*pcm.sampleRate),endSample:Math.ceil(entry.end*pcm.sampleRate),leadingSilenceSamples:0,trailingSilenceSamples:0,boundaryMethod:'alignment-v1'};
  const {startSample,endSample,leadingSilenceSamples,trailingSilenceSamples}=window,bytes=padPcmWave(slicePcmWave(master,startSample,endSample),leadingSilenceSamples,trailingSilenceSamples),hash=sha(bytes),uri='/audio/cast/clips/'+hash+'.wav';
  ...
  entry.clip={uri,sha256:hash,bytes:bytes.length,duration:(endSample-startSample+leadingSilenceSamples+trailingSilenceSamples)/pcm.sampleRate,sourceSha256:entry.sha256,sourceWavSha256:wavHash,...window,sampleRate:pcm.sampleRate,channels:pcm.channels};
 }
}
...
const result={schema:'eq.independent-cast-clips.v1',at:new Date().toISOString(),algorithm:'Pronunciation uses complete isolated word recordings, preserving every PCM16 sample with no word cuts. Passages retain alignment cuts. Independent RIFF files; no resampling or synthesis in this derivation.',providerRequests:0,manifestSha256:sha(await fs.readFile(root+'/manifest.json')),entries:manifest.entries.length,files:[...files.values()],inputs:[...inputs.values()],missing,status:missing.length?'INCOMPLETE':'COMPLETE'};
